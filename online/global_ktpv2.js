...
window.addEventListener('load', () => {
    ...
    var host = require('../online/server.json').ktpv2.hostname;
    var port = require('../online/server.json').ktpv2.port;
    ...
    client.on("data", chunk => {
        ...
        while (!received.isFinished()) {
            ...
            datas.forEach((data) => {
                ...
                if (typeof inf['t'] != "string") {
                    crash(new Error("Invalid JSON data"));
                }
                if (!client.initialized) {
                    switch (inf['t']) {
                        case "init":
                            if (inf['name'] !== "KTPv2-compatible Server") {
                            ...
                            if (inf.version.endsWith("-iridium-ktpv2")) {
                                document.getElementById("servername").innerText = "Iridium for KartikX " + inf.version.substr(0, inf.version.length - "-iridium-ktpv2".length);
                                document.getElementById("servericon").src = "../logo/iridium.png";
                            } else {
                                document.getElementById("servername").innerText = "Cobalt.rs Server " + inf.version;
                    ...
                    switch (inf['t']) {
                        ...
                        default:
                            ...
                            if ((inf['t'] === "ipc" && inf['action'] === "Pong") || inf['t'] === "pong") {
                                pingEnd = new Date();
